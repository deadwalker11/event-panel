...
header("location:staffHomepage.php?q=3");
}
}
if(isset($_SESSION['key'])){
if(@$_GET['semail'] && $_SESSION['key']=='sunny7785068889') {
$semail=@$_GET['semail'];
$r1 = mysqli_query($con,"DELETE FROM scores WHERE email='$semail' ") or die('Error');
$r2 = mysqli_query($con,"DELETE FROM history WHERE email='$semail' ") or die('Error');
$result = mysqli_query($con,"DELETE FROM staff_tb WHERE email='$semail' ") or die('Error');
...
$demail=@$_GET['demail'];
$r1 = mysqli_query($con,"DELETE FROM scores WHERE email='$demail' ") or die('Error');
$r2 = mysqli_query($con,"DELETE FROM history WHERE email='$demail' ") or die('Error');
$result = mysqli_query($con,"DELETE FROM participate WHERE email='$demail' ") or die('Error');
header("location:staffHomepage.php?q=1");
}
}
//remove quiz
if(isset($_SESSION['key'])){
...
header("location:staffHomepage.php?q=5");
}
}
...
header("location:staffHomepage.php?q=4&step=2&eid=$id&n=$total");
}
}
...
header("location:staffHomepage.php?q=0");
}
}
...
$q=mysqli_query($con,"SELECT * FROM scores WHERE email='$email'" )or die('Error161');
$rowcount=mysqli_num_rows($q);
if($rowcount == 0)
{
$q2=mysqli_query($con,"INSERT INTO scores VALUES('$email','$s',NOW())")or die('Error165');
}
else
{
while($row=mysqli_fetch_array($q) )
...
$sun=$s+$sun;
$q=mysqli_query($con,"UPDATE `scores` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
}
